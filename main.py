...
def user_interaction():
    ...
    salary_range = ""
    while True:
        salary_input = input("Введите диапазон зарплат в RUR (например: 100000-200000 или оставьте пустым): ").strip()
        if not salary_input:
            salary_range = ""
            break
        if "-" not in salary_input:
            print("Пожалуйста, используйте формат: минимальная-максимальная (например: 100000-200000)")
            continue
        try:
            ...
            print("Пожалуйста, введите числа в формате: минимальная-максимальная (например: 100000-200000)")

    # Получение и сохранение вакансий
    print("\nПолучаем вакансии с HeadHunter...")
    vacancies_data = hh_api.get_vacancies(search_query, per_page=top_n*2)  # Берем больше вакансий для фильтрации
    ...
    filtered_vacancies = filter_vacancies(all_vacancies, filter_words) if filter_words else all_vacancies
    ranged_vacancies = get_vacancies_by_salary(filtered_vacancies, salary_range) if salary_range else filtered_vacancies
