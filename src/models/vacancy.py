...
class Vacancy:
    ...
    __slots__ = ['_title', '_url', '_salary_from', '_salary_to', '_currency', '_description', '_requirements']

    def __init__(self, title: str, url: str, salary_from: Optional[int] = None,
                 salary_to: Optional[int] = None, currency: Optional[str] = None,
                 description: Optional[str] = None, requirements: Optional[str] = None):
        self._title = title
        self._url = url
        self._salary_from = salary_from
        self._salary_to = salary_to
        self._currency = currency.lower() if currency else None  # Приводим к нижнему регистру
    ...
    def _validate_data(self) -> None:
        ...
        if not isinstance(self._url, str) or not self._url.startswith(('http://', 'https://')):
            raise ValueError("URL вакансии должен быть валидной ссылкой")

        self._salary_from = self._validate_salary(self._salary_from)
        self._salary_to = self._validate_salary(self._salary_to)

        if self._salary_to and self._salary_from and self._salary_to < self._salary_from:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
                ...
                self._salary_from == other._salary_from and
                self._salary_to == other._salary_to and
